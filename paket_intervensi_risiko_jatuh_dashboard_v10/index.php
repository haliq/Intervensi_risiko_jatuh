<?php
/**
 * Modul Intervensi Pencegahan Risiko Jatuh - V8 Perbaikan Simpan NPK
...
if ($mode === 'risiko_tinggi') {
    ...
    if (!$sources) {
        ...
        $sql_risiko = "SELECT 
                ...
                COALESCE(irj.total_intervensi,0) AS total_intervensi,
                irj.id_intervensi, irj.no_form_intervensi, irj.tanggal_intervensi
            FROM (".$union.") x
            ...
            LEFT JOIN (
                ...
                GROUP BY no_rawat
            ) irj ON irj.no_rawat=x.no_rawat
        ...
        if ($union_year !== '') {
            $sql_monthly = "SELECT DATE_FORMAT(x.tanggal_skrining,'%Y-%m') AS bulan,
                                   ...
                                   SUM(CASE WHEN irj.no_rawat IS NULL THEN 0 ELSE 1 END) AS sudah_intervensi
                            FROM (".$union_year.") x
...
<body><main class="wrap">
  <div class="top">
    ...
    <div class="nav">
      <a class="btn gray" href="<?=rj_h(rj_url(array('mode'=>'daftar')))?>">Daftar Intervensi</a>
      <a class="btn green" href="<?=rj_h(rj_url(array('mode'=>'form')))?>">+ Form Baru</a>
    </div>
  </div>
      ...
      <tr>
        ...
        <td class="actions">
          <?php if ((int)$row['total_intervensi'] > 0 && (int)$row['id_intervensi'] > 0): ?>
            <a class="btn" target="_blank" href="<?=rj_h(rj_url(array('mode'=>'cetak','id'=>$row['id_intervensi'])))?>">Cetak</a>
            <a class="btn gray" href="<?=rj_h(rj_url(array('mode'=>'edit','id'=>$row['id_intervensi'])))?>">Edit</a>
          <?php else: ?>
            <a class="btn orange" href="<?=rj_h(rj_url(array('mode'=>'form','no_rawat'=>$row['no_rawat'])))?>">Buat Intervensi</a>
          <?php endif; ?>
        </td>
      </tr>
      <?php endforeach; else: ?>
